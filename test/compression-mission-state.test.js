...
function makeSqlJson(rowsByQuery) {
  return (sql, params) => {
    ...
    if (/FROM research_findings/.test(sql)) {
      return rowsByQuery.findings ?? [];
    }
    if (/FROM handoffs/.test(sql)) {
      return rowsByQuery.handoffs ?? [];
    }
    if (/FROM validation_verdicts/.test(sql)) {
      return rowsByQuery.verdicts ?? [];
    }
    if (/FROM cost_entries/.test(sql)) {
      return rowsByQuery.costs ?? [];
    }
    return [];
  };
}
...
describe('compressMissionState', () => {
  ...
  it('aggregates worker handoffs into the summary', () => {
    const queries = [],
      ...
      handoffQuery = queries.find((q) => /FROM handoffs/.test(q.sql));
