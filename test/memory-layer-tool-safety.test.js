...
describe('memory tool renderer safety', () => {
  it('keeps full tool content while limiting terminal result previews', () => {
    const text = Array.from({ length: 20 }, (_, i) => `line ${i + 1}`).join('\n'),
      result = normalizeToolResult({ content: [{ type: 'text', text }], details: { full: true } }),
      theme = { fg: (_name, value) => value },
      collapsed = renderCompactToolResult(result, { expanded: false }, theme).render(120).join('\n'),
      expanded = renderCompactToolResult(result, { expanded: true }, theme).render(120).join('\n');
  ...
  it('blocks accidental memory-get content from another project unless explicitly allowed', async () => {
    const tool = captureNamedTool(
      ...
      blocked = await tool.execute('id', { id: 2 }, undefined, vi.fn(), {}),
      allowed = await tool.execute('id', { id: 2, allow_cross_project: true }, undefined, vi.fn(), {});
  ...
  it('keeps memory-code streaming updates renderable during indexing', async () => {
    const onUpdate = vi.fn(),
      tool = captureTool(registerCodeTools, {
        mem: vi.fn(),
      ...
      result = await tool.execute('id', { mode: 'reindex-repo', path: '.', name: 'app' }, undefined, onUpdate, {});
  ...
  it.each([
    ...
    const tool = captureTool(registerCodeTools, {
      ...
      result = await tool.execute('id', params, undefined, vi.fn(), {});
    expectRenderable(result);
  });
  ...
  it('caps memory-code outline details before returning them to the agent', async () => {
    const classes = Array.from({ length: 30 }, (_classItem, classIndex) => ({
      ...
      standalone = Array.from({ length: 120 }, (_, index) => ({ name: `fn${index}`, kind: 'function' })),
      tool = captureTool(registerCodeTools, {
      ...
      result = await tool.execute('id', { mode: 'outline', repo: 'app', file: 'src' }, undefined, vi.fn(), {});
  ...
  it('unwraps and caps enveloped memory-code outline results before formatting', async () => {
    const standalone = Array.from({ length: 120 }, (_item, index) => ({
      ...
      result = await tool.execute('id', { mode: 'outline', repo: 'app', file: 'src/large.js' }, undefined, vi.fn(), {}),
  ...
  it('infers memory-code repo from current working directory', async () => {
    const mem = vi.fn().mockResolvedValue({
      ...
      result = await tool.execute('id', { mode: 'search', query: 'context command' }, undefined, vi.fn(), {}),
      text = result.content.find((item) => item.type === 'text').text;
  ...
  it('infers memory-code repo when only one indexed repo is available', async () => {
    const mem = vi.fn().mockResolvedValue({
      ...
      result = await tool.execute('id', { mode: 'search', query: 'rankObservations' }, undefined, vi.fn(), {});
