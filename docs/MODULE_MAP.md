...
| Agent intelligence | `src/agent-intel/` | `src/agent-intel/preflight.js` | Agent-facing coding intelligence orchestration: preflight checks, agent-pack planning, audit-diff, blast radius orchestration, dupes, runtime-ingest, stale-flags, and symbol-enrichment. | Code-index read model, memory-domain search, doc-index. | Pi extension state, raw SQL, mutation of memory or code indexes. |
