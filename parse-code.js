...
const path = require('path'),
  fs = require('fs'),
  { SKIP_CALLEE_NAMES } = require('./utils');
  ...
  async function init() {
    ...
    _initPromise = (async () => {
      try {
        ...
        {
          const grammarEntries = Object.entries(LANGUAGE_MAP).map(([, config]) => [
              config.parserKey,
              config.grammarFile,
            ]),
...

          {
                ...
                if (node.type === 'element' || node.type === 'script_element' || node.type === 'style_element') {
                  ...
                  if (startTag) {
                    const tagName = getTagName(startTag).toLowerCase(),
                      ...
                      sig = source.substring(
                        startTag.startIndex,
                        Math.min(startTag.endIndex, startTag.startIndex + 200),
                      ),
                      ...
                      isCustom = (() => {
                        ...
                        return (
                          (tagName.includes('-') && !_STANDARD_HTML_TAGS.has(tagName)) ||
                          /^[A-Z]/.test(getTagName(startTag))
                        );
                      })(),
                      resAttrs = (() => {
                        if (isCustom) {
                  ...
                  if (node.type === 'script_element' && rawText) {
                    ...
                    if (body) {
                      const startTag2 = node.children.find((c) => c.type === 'start_tag'),
                        sl2 = rawText.startPosition.row + 1,
            ...
            function extractCalleesFromContent(filePath, content) {
              ...
              const ext = path.extname(filePath).toLowerCase(),
                ...
                parser = !(!langConfig || langConfig.languageName === 'sql')
                  ? _parsers[langConfig.parserKey]
                  : undefined,
                tree =
                  !(!langConfig || langConfig.languageName === 'sql') && parser ? parser.parse(content) : undefined,
                result =
                  !(!langConfig || langConfig.languageName === 'sql') && parser
                    ? _walkCallees(tree.rootNode, SKIP_CALLEE_NAMES)
                    : undefined;
            ...
            function _fallbackExtractSymbols(filePath, content) {
              const ext = path.extname(filePath).toLowerCase(),
                symbols = [],
                seen = new Set(),
                jsTsExts = new Set(['.js', '.jsx', '.mjs', '.cjs', '.ts', '.mts', '.cts', '.tsx']);
            ...
            function _extractSqlSymbolsRegex(filePath, source) {
              const symbols = [],
                ...
                patterns = [
                  ...
                  {
                    re: /\bCREATE\s+PROCEDURE\s+([`\[\]"']?\w+[`\[\]"']?(?:\.[`\[\]"']?\w+[`\[\]"']?)?)/gi,
                    kind: 'procedure',
                  },
                ],
                selectRe = (() => {
            ...
            function _extractSqlSymbols(filePath, sourceStr, parser) {
              ...
              function walk(node) {
                if (node.type in SQL_STATEMENT_MAP) {
                  ...
                  if (!name) {
                    name =
                      { select: 'SELECT', insert: 'INSERT', update: 'UPDATE', delete: 'DELETE' }[node.type] ||
                      'UNKNOWN';
                  }
