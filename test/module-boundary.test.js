...
const { rankObservations } = require('../src/memory-domain/search'),
  { evaluateTrustSync } = require('../src/trust-sync/trust-policy');
  ...
  // ─── Task 3: Trust sync failure isolation ───────────────────────────
  {
    ...
    // ─── Task 4: Code analyzer failure isolation ────────────────────────
    {
      const { runAnalyzer, scopedError } = require('../src/code-analysis/analyzer-runner'),
        graph = require('../src/code-analysis/graph'),
        quality = require('../src/code-analysis/quality');
      ...
      // ─── Task 5: Formatter failure isolation ────────────────────────────
      {
        const {
          ...
          } = require('../src/platform/protocol/llm-format'),
          { compactResponse, expandResponse } = require('../src/platform/protocol/compact-format');
