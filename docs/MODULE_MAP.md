...
| Agent intelligence | `src/agent-intel/` | `src/agent-intel/preflight.js` (primary) plus siblings `blast.js`, `dupes.js`, `symbol-enrichment.js`, `audit-diff.js`, `runtime-ingest.js`, `stale-flags.js` | Agent-facing coding intelligence orchestration: preflight checks, agent-pack planning, audit-diff, blast radius orchestration, dupes, runtime-ingest, stale-flags, and symbol-enrichment. | Code-index read model, memory-domain search, doc-index. | Pi extension state, raw SQL, mutation of memory or code indexes. |
...
| HTTP server (Aurex domain) | `src/http/` | `src/http/server.js` | REST API server for Aurex domain model (missions, milestones, working units, handoffs, contracts, verdicts, broadcasts, findings, sessions, costs, retry, compression, checkpoints, settings), code indexing/analysis endpoints, and `POST /dispatch` (gateway dispatch for daemon mode). Route matching, JSON body parsing, error helpers. | Platform repositories (`aurex`), `db.js` for startup, CLI gateway for `/dispatch`. | Business logic, raw SQL, Pi extension state. |
| MCP transport adapter | `src/mcp/` | `src/mcp/server.js` | MCP stdio server: tool catalog (`tools.js`), `CallTool` → `gateway.dispatch`, result translation (`translate-result.js`), project detection via hooks-engine + project-db. Second transport alongside Pi; tools only (no hooks). | `src/cli/gateway`, `src/hooks-engine/project`, `src/platform/project-db`, MCP SDK. | Pi extension state, hook logic, feature business-logic internals, raw SQL. |
| Platform project DB reads | `src/platform/` | `src/platform/project-db.js` | Cached `getKnownRepos()` / `getKnownProjects()` reads (5 min TTL, Pi `REPO_CACHE_TTL` parity). Shared by MCP server, Claude Code dispatch client, and hook handlers for monorepo cwd resolution. | `db.js` read queries only. | Feature mutations, Pi extension state. |
...
| Shared hooks engine | `src/hooks-engine/` | `src/hooks-engine/index.js` | Transport-agnostic, pure-JS extraction of the Pi extension's hook logic: pattern matcher, prompt classifiers, context builder, preflight assembly, passive capture, session summary, project detection, guardrail utilities, and `tool_response` parsing. Consumed by both the Pi extension and the Claude Code bridge. | Node built-ins and `constants` only. | Pi `ExtensionAPI`, `extensions/` internals, raw SQL, transport specifics. |
| Claude Code hooks bridge | `src/claude-code/` | `src/claude-code/hooks.js` | Claude Code CLI lifecycle bridge: hook router, per-session state store (`state-store.js` + `gc`), dispatch client (direct + daemon POST), daemon lifecycle (`daemon.js`), context injection, transcript reader, lifecycle/tool-call handlers (SessionStart, UserPromptSubmit, Stop, SessionEnd, PreToolUse guardrails, PostToolUse tracking + tool-state mirroring), and the installer surface (`install.js`/`uninstall.js`/`doctor.js` — `.mcp.json` + `.claude/settings.json` config writer, sentinel-based uninstall, install self-check, optional `--daemon`). | `src/hooks-engine`, `src/platform/project-db`, the CLI dispatch gateway, `db.js`, HTTP `/dispatch` when daemon mode is active. | Pi `ExtensionAPI` / `extensions/` state, feature business-logic internals, raw SQL. |
| Hermes Agent hooks bridge | `src/hermes/` | `src/hermes/hook.js` | Hermes Agent integration: shell-hook dispatcher (`hook.js` — stdin JSON payload, `pre_tool_call` read guardrail reusing hooks-engine guardrails, `post_tool_call` fire-and-forget trust sync, `on_session_end` best-effort close; hooks fail open) and installer surface (`install.js`/`uninstall.js`/`doctor.js` — line-based `$HERMES_HOME/config.yaml` editor in `config-editor.js` wiring `mcp_servers.<name>` + `hooks:` + `hooks_auto_accept`, consent in `shell-hooks-allowlist.json`, bundled `hermes/SKILL.md`). | `src/hooks-engine`, `src/code-index/scanner`, `src/cli/gateway`, `db.js` (via `LAPIS_HOME`). | Pi `ExtensionAPI` / `extensions/` state, Claude Code hook specifics, raw SQL. |
