...
const { DEDUP, CAPTURE_PASSIVE } = require('../constants');

function save(deps, args) {
  const { sqlJson, jsonErrNoExit, insertObservation, insertObservationRelation, softDeleteObservation, checkDuplicate, findLatestSession } = deps;
  ...
  if (!force) {
    ...
    if (dupes.potential_duplicates.length > 0) {
      ...
      if (bestMatch.similarity >= dedupCfg.auto_merge_threshold) {
        ...
        insertObservationRelation({ sourceId: newId, targetId: bestMatch.id, relation: 'duplicate', confidence: bestMatch.similarity });
