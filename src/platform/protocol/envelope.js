...
const { execSync } = require('child_process'),
  path = require('path'),
  fs = require('fs'),
  { FRESHNESS_CACHE_TTL_MS, CONFIDENCE_DEFAULTS } = require('../../../constants'),
  // ══════════════════════════════════════════════════════════
  // FRESHNESS CACHE  (module-level, 60s TTL)
  // ══════════════════════════════════════════════════════════

  _freshnessCache = new Map();
  ...
  const _confidenceCalculators = {
      ...
      getExtractionCandidates(data) {
        const candidates = data?.candidates || [],
          maxScore = !(candidates.length === 0)
            ? Math.max(...candidates.map((c) => c.extraction_score || 0))
            : undefined;
      ...
      getProvenance(data) {
        const commits = data?.commits || [],
          classified = !(commits.length === 0)
            ? commits.filter((c) => c.classification !== 'unknown').length
            : undefined;
        if (commits.length === 0) {
          return 1.0;
        }
