...
| Claude Code hooks bridge | `src/claude-code/` | `src/claude-code/hooks.js` | Claude Code CLI lifecycle bridge: hook router, per-session state store, dispatch client, context injection, transcript reader, lifecycle/tool-call handlers (SessionStart, UserPromptSubmit, Stop, SessionEnd, PreToolUse guardrails, PostToolUse tracking + tool-state mirroring), and the installer surface (`install.js`/`uninstall.js`/`doctor.js` — `.mcp.json` + `.claude/settings.json` config writer, sentinel-based uninstall, install self-check). | `src/hooks-engine`, the CLI dispatch gateway, `db.js`, MCP result translation. | Pi `ExtensionAPI` / `extensions/` state, feature business-logic internals, raw SQL. |
