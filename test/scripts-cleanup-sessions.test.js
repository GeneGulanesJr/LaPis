...
describe('scripts/cleanup-sessions', () => {
  ...
  it('should respect keep-last and prune sessions', () => {
    const deletedSessions = [],
      ...
      result = cleanupSessions(deps, { keepLast: 10, project: 'test-proj', yes: true });
  ...
  it('should refuse to delete when project has 5 or fewer sessions', () => {
    const deps = {
      ...
      result = cleanupSessions(deps, { keepLast: 10, project: 'small-proj', yes: true });
    expect(result.ok).toBe(true);
  ...
  it('should return dry-run message without --yes', () => {
    const deps = {
      ...
      result = cleanupSessions(deps, { keepLast: 10 });
  ...
  it('should filter to a specific project when --project is set', () => {
    const deps = {
      ...
      result = cleanupSessions(deps, { keepLast: 10, project: 'proj-b', yes: true });
    expect(result.ok).toBe(true);
