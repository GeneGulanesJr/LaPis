...
| Claude Code hooks bridge | `src/claude-code/` | `src/claude-code/hooks.js` | Claude Code CLI lifecycle bridge: hook router, per-session state store, dispatch client, context injection, transcript reader, and lifecycle/tool-call handlers (SessionStart, UserPromptSubmit, Stop, SessionEnd, PreToolUse guardrails, PostToolUse tracking + tool-state mirroring). | `src/hooks-engine`, the CLI dispatch gateway, `db.js`, MCP result translation. | Pi `ExtensionAPI` / `extensions/` state, feature business-logic internals, raw SQL. |
